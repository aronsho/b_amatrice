...
# local imports
from functions.general_functions import transform_n
...
def mac_one_dimension(
    ...
    if not np.all(np.diff(order) >= 0*np.diff(order)[0]):
        warnings.warn("The order variable was not ordered, it was ordered")
    ...
    for ii in range(n_realizations):
        ...
        b_vec,  std_b_vec, n_m_loop = values_from_partitioning(
        ...
        if transform is True:
            ...
            b_all = estimate_b(
                mags_sorted, mc, delta_m=delta_m, method=method)
...
def cut_constant_value(
        values: np.ndarray,
        delta_value: float,
        offset: float | None = None) -> tuple[list[int], list[np.ndarray]]:
    """
    Cuts the array into subsamples with a maximum span of delta_value.

    Args:
        values:        Array of numeric values.
        delta_value:   Maximum allowed span (max-min) inside a subsample.
            Units have to match those of values.
        offset:        value difference from the first element where the first
            cut occurs It should be between zero and delta_value. If None, it
            is set to zero. Units have to match those of values.
    Returns:
        ...
        subsamples: The resulting subsamples.
    """
    ...
    if offset is not None:
        try:
            _ = values[0] + delta_value
            _ = values[0] + offset
        except Exception:
            raise TypeError(
                "values, delta_value, and offset must have compatible units.")
        if offset >= delta_value:
            raise ValueError("Offset must be smaller than delta_value.")
    elif offset is None:
        offset = values[0] - values[0]

    # generate threshold values
    max_val = values[-1]
    min_val = values[0]
    thresholds = np.arange(min_val + offset, max_val +
                           delta_value, delta_value)

    # convert thresholds to indices
    idx = [int(np.searchsorted(values, t)) for t in thresholds]
    idx = [i for i in idx if 0 < i < len(values)]

    subsamples = np.array_split(values, idx)
    return np.array(idx), subsamples
